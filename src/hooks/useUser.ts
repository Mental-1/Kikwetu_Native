...
import { setUserPlan } from "@/stores/useAppStore";
...
export const useUser = (): UseUserReturn => {
  ...
  const fetchUserData = useCallback(async () => {
    try {
      ...
      // Fetch subscription in background and update MMKV
      userService.getSubscription().then((subscriptionResponse) => {
        if (subscriptionResponse.success && subscriptionResponse.data) {
          setSubscription(subscriptionResponse.data);
          setUserPlan({
            maxListings: subscriptionResponse.data.maxListings,
            usedListings: subscriptionResponse.data.usedListings,
            planName: subscriptionResponse.data.planName,
          });
        }
      }).catch((err) => {
  ...
  const updateProfile = useCallback(
    async (data: UpdateProfileData): Promise<UserProfile | null> => {
      try {
  ...
  const updatePreferences = useCallback(
    async (data: UpdatePreferencesData): Promise<UserPreferences | null> => {
      try {
  ...
  const uploadAvatar = useCallback(
    async (imageData: FormData): Promise<string | null> => {
      try {
  ...
  const confirmPhoneVerification = useCallback(
    async (verificationId: string, code: string): Promise<boolean> => {
      try {
        const response = await userService.confirmPhoneVerification(
          verificationId,
          code,
        );
        if (response.success) {
